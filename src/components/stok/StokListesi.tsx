...
export default function StokListesi({ stoklar, yukleniyor, onDuzenle, onSil }: Props) {
  const [arama, setArama] = useState('')

  const filtrelenmis = stoklar.filter(
    (s) =>
      s.ad.toLowerCase().includes(arama.toLowerCase()) ||
      s.kod.toLowerCase().includes(arama.toLowerCase()) ||
      (s.tip ?? '').toLowerCase().includes(arama.toLowerCase())
  )
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50 text-left text-gray-500 font-medium">
                ...
                <th className="px-4 py-3">Kalınlık</th>
